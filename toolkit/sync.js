...
async function checkIfAppIsInstalled(appName){
    const mds = await axios.get(`http://127.0.0.1:${rpcPort}/mds`).catch(() => {
        console.log('Skipping automatic environment setup, please check RPC is running on port 9005.');
...
function installMinmaApp() {
    ...
    const command = `mds action:install file:${__dirname}/../minidapp/${file} trust:write`;
...
(async () => {
    const dAppConf = fs.readFileSync('./public/dapp.conf', 'utf-8');
